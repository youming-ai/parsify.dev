...
export const FilePreview: React.FC<FilePreviewProps> = ({
	...
	const isTextFile = useMemo(() => {
		...
		const textExtensions = ['.txt', '.json', '.csv', '.md', '.html', '.xml', '.js', '.css', '.yaml', '.yml'];

		return textMimeTypes.includes(file.type) || textExtensions.some((ext) => fileName.endsWith(ext));
	}, [file.type, file.name]);
		...
		if (isImageFile && content) {
			return (
				<div className="flex items-center justify-center p-4">
					<img src={content} alt={file.name} className="max-h-full max-w-full rounded object-contain" />
				</div>
			);
		}
