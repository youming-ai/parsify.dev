			...
			<CardContent className="flex flex-col items-center justify-center p-8 text-center">
				...
				{children || (
					<div className="space-y-4">
						...
						{accept.length > 0 && (
							<p className="text-gray-400 text-xs dark:text-gray-500">Accepted formats: {accept.join(', ')}</p>
						)}

						{maxSize && (
							<p className="text-gray-400 text-xs dark:text-gray-500">Max file size: {formatFileSize(maxSize)}</p>
						)}
					</div>
				)}
			</CardContent>
