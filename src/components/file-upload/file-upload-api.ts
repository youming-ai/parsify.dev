import type { FileUploadOptions, FileUploadProgress, FileUploadResponse, UploadedFile } from './file-upload-types';
...
export class FileUploadApiService {
	...
	private async uploadSingle(
		...
		while (attempt < maxAttempts) {
			try {
				...
				if (
					attempt >= maxAttempts ||
					!this.config.retryConfig?.retryCondition?.(error instanceof Error ? error : new Error('Unknown error'))
				) {
					throw error;
				}
	...
	private async addToQueue(uploadTask: () => Promise<FileUploadResponse>): Promise<FileUploadResponse> {
		return new Promise((resolve, reject) => {
