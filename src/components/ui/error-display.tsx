	...
	const content = (
		<div className={cn(baseClasses, variantClasses[variant], className)}>
			...
			<div className="flex-1 min-w-0 space-y-2">
				<p className="text-sm font-medium leading-none">{errorMessage || 'An unexpected error occurred'}</p>

				{showDetails && errorStack && (
					<details className="text-xs">
						<summary className="cursor-pointer text-muted-foreground hover:text-foreground">Error details</summary>
						<pre className="mt-2 whitespace-pre-wrap text-muted-foreground overflow-x-auto">{errorStack}</pre>
					</details>
				)}
