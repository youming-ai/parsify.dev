...
export function RegexTester({ value, onChange, height = 400, className }: RegexTesterProps) {
	...
	const loadPattern = (selectedPattern: string) => {
		const patternObj = commonPatterns.find((p) => p.pattern === selectedPattern);
		if (patternObj) {
				...
				<Card>
					...
					<CardContent>
						{result.isValid ? (
									...
									<Tabs defaultValue="matches" className="w-full">
										...
										<TabsContent value="matches" className="space-y-2">
											{result.matches.map((match, index) => (
												<div key={`match-${match.index}-${index}`} className="p-3 bg-gray-50 rounded border">
													<div className="flex items-center gap-2 mb-2">
														<Badge variant="outline">Match {index + 1}</Badge>
														<span className="text-sm text-gray-600">Index: {match.index}</span>
													</div>
													<div className="font-mono text-sm bg-white p-2 rounded border">{match.match}</div>
													{match.groups.length > 0 && (
														<div className="mt-2 space-y-1">
															<span className="text-sm font-medium">Groups:</span>
															{match.groups.map((group, groupIndex) => (
																<div
																	key={`group-${match.index}-${groupIndex}-${group}`}
										...
										<TabsContent value="groups" className="space-y-2">
											{result.matches.map((match, index) => (
												<div key={`groups-match-${match.index}-${index}`} className="p-3 bg-gray-50 rounded border">
													...
													{match.groups.length > 0 ? (
														<div className="space-y-2">
															{match.groups.map((group, groupIndex) => (
																<div
																	key={`group-item-${match.index}-${groupIndex}-${group}`}
														...
														<div className="text-gray-500 text-sm">No capture groups</div>
													)}
												</div>
											))}
										</TabsContent>
											...
											<div className="font-mono text-sm whitespace-pre-wrap p-3 bg-gray-50 rounded border">
												{(() => {
													...
													for (const match of result.matches) {
														...
														offset += '<mark class="bg-yellow-200 px-1 rounded">'.length + '</mark>'.length;
													}
							...
							<div className="text-red-600 text-center py-4">{result.error || 'Invalid pattern'}</div>
						)}
					</CardContent>
				</Card>
