import type { CodeExecutionRequest, CodeExecutionResult, CodeFormatOptions, CodeLanguage } from './code-types';
...
class CodeApiService {
	...
	// Format code
	async formatCode(code: string, language: CodeLanguage, options: CodeFormatOptions): Promise<string> {
		try {
	...
	async cancelExecution(executionId: string): Promise<void> {
		try {
			const response = await fetch(`${this.baseUrl}/execute/${executionId}/cancel`, {
				method: 'POST',
			});
	...
	async getExecutionStatus(executionId: string): Promise<{
		...
		try {
			const response = await fetch(`${this.baseUrl}/execute/${executionId}/status`, {
				method: 'GET',
			});
