...
export function CodeToolComplete({ className }: CodeToolCompleteProps) {
	...
	React.useEffect(() => {
		const welcomeLine = createTerminalLine(`Welcome to Code Runner! Selected language: ${languageConfig.name}`, 'info');
	...
	const handleTemplateSelect = (template: CodeTemplate) => {
		...
		const templateLine = createTerminalLine(`Loaded template: ${template.name} (${template.language})`, 'info');
					...
					<CardContent className="p-4">
						<div className="flex flex-wrap items-center gap-4">
							<div className="flex items-center gap-2">
								...
								<LanguageSelector selectedLanguage={language} onLanguageChange={setLanguage} compact={true} />
							</div>
									...
									<div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
										{templates.map((template) => (
											<Card key={template.id} className="cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800">
												...
												<CardContent>
													<p className="mb-2 text-gray-600 text-xs dark:text-gray-400">{template.description}</p>
													<div className="flex items-center justify-between">
