...
import { ChevronDown, ChevronRight, Clock, Copy, Download, Send, Terminal as TerminalIcon, Trash2 } from 'lucide-react';
					...
					<div className="space-y-1">
						{limitedLines.map((line, index) => (
							<div key={line.id} className="flex items-start gap-2">
								...
								<span className={cn('w-4 flex-shrink-0', getLineColor(line.type))}>{getLineIcon(line.type)}</span>

								<span className={cn('flex-1 break-words', getLineColor(line.type))}>{line.content}</span>
							</div>
						))}
						<div ref={linesEndRef} />
					</div>
...
export const createTerminalLine = (content: string, type: TerminalLine['type'] = 'output'): TerminalLine => ({
