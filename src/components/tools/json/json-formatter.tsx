...
export function JsonFormatter({
	...
	return (
		<div className={cn('space-y-4', className)}>
					...
					<CardContent className="space-y-4">
						<div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
							...
							<div className="space-y-2">
								<label className="font-medium text-gray-700 text-sm">Indent Size: {formatOptions.indent}</label>
								<div className="flex items-center gap-2">
									<input
										...
										onChange={(e) => handleOptionChange('indent', Number.parseInt(e.target.value, 10))}
										className="flex-1"
									/>
			...
			{/* Format Controls */}
			<div className="flex items-center gap-2">
				<Button onClick={handleFormat} disabled={isFormatting || !input.trim()} className="flex items-center gap-2">
