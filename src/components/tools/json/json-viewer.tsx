...
function JsonTreeItem({ node, level, showLineNumbers, lineNumber, onCopy }: JsonTreeItemProps) {
	...
	const _isLastChild = node.key.endsWith(']') && !Number.isNaN(Number.parseInt(node.key.split('[').pop() || '', 10));
	...
	return (
		<div className={cn('font-mono text-sm', 'select-text')}>
			<div className={cn('flex items-start gap-1', 'rounded px-1 hover:bg-gray-50')}>
				{showLineNumbers && (
					<span className="mr-2 w-8 select-none text-right text-gray-400 text-xs">{lineNumber.current++}</span>
				)}
	...
	return (
		<div className={cn('space-y-4', className)}>
			...
			<div className="flex items-center justify-between border-b pb-3">
				...
				{copyable && (
					<div className="flex items-center gap-2">
						<Button variant="outline" size="sm" onClick={handleCopyAll} className="flex items-center gap-1">
			...
			<div className="max-h-96 overflow-auto rounded-lg border bg-gray-50 p-4">
				...
				{treeData.length === 0 && <div className="py-8 text-center text-gray-500">No data to display</div>}
			</div>
