...
import { AlertCircle, CheckCircle, Settings, Shield, Upload, XCircle } from 'lucide-react';
...
export function DataValidator({ onValidationComplete, className }: DataValidatorProps) {
	...
	const [dataFormat, setDataFormat] = React.useState<'json' | 'csv' | 'xml' | 'yaml' | 'form'>('json');
	...
	const updateRule = (id: string, updates: Partial<ValidationRule>) => {
		setValidationRules((prev) => prev.map((rule) => (rule.id === id ? { ...rule, ...updates } : rule)));
	};
						...
						try {
							...
							switch (rule.type) {
								...
								case 'pattern':
									...
									break;
								case 'length':
									if (rule.value && rule.operation && (rule.operation === 'min' || rule.operation === 'max')) {
	...
	return (
		<div className={className}>
			<div className="space-y-6">
					...
					<CardContent className="space-y-4">
						<div className="grid md:grid-cols-2 gap-4">
							...
							<div className="space-y-2">
								...
								<Button variant="outline" onClick={() => loadSampleData(dataFormat)} className="w-full">
						...
						<div className="space-y-4">
							{validationRules.length === 0 ? (
								<Alert>
									...
									<AlertDescription>
										Add validation rules to check your data. You can use predefined templates or create custom rules.
									</AlertDescription>
								</Alert>
							) : (
											...
											<div className="grid md:grid-cols-3 gap-2">
												...
												{(rule.type === 'length' || rule.type === 'range') && (
													<Select
														...
														onValueChange={(value: any) => updateRule(rule.id, { operation: value })}
													>
														<SelectTrigger>
				...
				{/* Validation */}
				<Button onClick={validateData} disabled={isProcessing || validationRules.length === 0} className="w-full">
											...
											{result.warnings.length > 0 && (
												<div className="space-y-1">
													{result.warnings.map((warning, warningIndex) => (
														<div key={warningIndex} className="text-sm text-yellow-600 flex items-center gap-1">
