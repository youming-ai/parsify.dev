...
import { Download, Eye, FileSpreadsheet, Filter, Settings } from 'lucide-react';
...
export function CSVProcessor({ onProcessingComplete, onError, className }: CSVProcessorProps) {
	...
	const handleFilesDrop = (newFiles: File[]) => {
		const csvFiles = newFiles.filter((file) => file.type === 'text/csv' || file.name.toLowerCase().endsWith('.csv'));
		...
		<div className={className}>
			<div className="space-y-6">
				...
				<Card>
					...
					<CardContent>
						<FileUpload files={files} onFilesChange={setFiles} maxFiles={1} acceptedFormats={['csv']} />
						{files.length > 0 && (
									...
									<CardContent className="space-y-4">
										...
										<div className="grid md:grid-cols-2 gap-4">
											...
											<div className="space-y-2">
												...
												<Select onValueChange={(value: 'us' | 'eu') => updateOption('numberFormat', value)}>
													<SelectTrigger>
									...
									<CardContent className="space-y-4">
										<div className="grid md:grid-cols-2 gap-4">
											...
											<div className="space-y-2">
												...
												<Select onValueChange={(value: 'asc' | 'desc') => updateOption('sortOrder', value)}>
													<SelectTrigger>
				...
				{csvData.length === 0 && (
					<Alert>
						...
						<AlertDescription>
							Upload a CSV file to start processing. Supported features include filtering, sorting, data transformation,
