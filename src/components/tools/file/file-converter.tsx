...
<root>${JSON.stringify(obj)}</root>`;
	...
	const jsonToCsv = (obj: any): string => {
		...
		if (Array.isArray(obj)) {
			...
			const csv = [headers.join(','), ...obj.map((row) => headers.map((h) => row[h]).join(','))].join('\n');
			return csv;
		}
		...
		<div className={className}>
			<div className="space-y-6">
				...
				{files.length > 0 && (
					<Card>
						...
						<CardContent className="space-y-4">
							...
							<Button onClick={handleConvert} disabled={!targetFormat || isConverting} className="w-full">
				...
				<Alert>
					...
					<AlertDescription>
						Max file size: {(maxFileSize / 1024 / 1024).toFixed(0)}MB. Supported formats: Images (JPG, PNG, WebP),
						Documents (PDF, TXT), Data files (JSON, CSV, XML). Conversion happens in your browser - files are never
						uploaded to servers.
					</AlertDescription>
				</Alert>
			</div>
