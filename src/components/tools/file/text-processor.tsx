...
export function TextProcessor({ value, onChange, height = 400, className }: TextProcessorProps) {
	...
	const handleFilesDrop = (newFiles: File[]) => {
		const textFiles = newFiles.filter(
			(file) => file.type.startsWith('text/') || file.name.match(/\.(txt|md|csv|json|xml|html|css|js|ts)$/i),
		);
							...
							<CardContent className="space-y-4">
								...
								<div className="grid md:grid-cols-2 gap-4">
									<div className="space-y-2">
										...
										<Select onValueChange={(value: SearchType) => updateOption('searchType', value)}>
											<SelectTrigger>
							...
							<CardContent className="space-y-4">
								<div className="grid md:grid-cols-2 gap-4">
									<div className="space-y-2">
										...
										<Select onValueChange={(value: TransformType) => updateOption('transformType', value)}>
											<SelectTrigger>
								...
								<div className="grid md:grid-cols-2 gap-4">
									<div className="space-y-2">
										...
										<Select onValueChange={(value: 'lf' | 'crlf' | 'cr') => updateOption('lineEndings', value)}>
											<SelectTrigger>
						...
						<Card>
							...
							<CardContent>
								<p className="text-gray-600">
									Additional formatting options will be available here, such as indentation, wrapping, and more advanced
							...
							{result.success ? (
								<>
									<div className="grid md:grid-cols-4 gap-4 text-sm">
										<div>
											<span className="font-medium">Original Length:</span> {result.stats.originalLength}
										</div>
										<div>
											<span className="font-medium">Processed Length:</span> {result.stats.processedLength}
										</div>
										<div>
											<span className="font-medium">Matches Found:</span> {result.stats.matchesFound}
										</div>
										<div>
