		...
		const observeMetrics = () => {
			...
			if (navigationEntry) {
				setMetrics((prev) => ({
					...
					domContentLoaded: Math.round(navigationEntry.domContentLoadedEventEnd - navigationEntry.fetchStart),
			...
			<div className="max-h-80 overflow-y-auto p-4">
					...
					<div className="space-y-2 text-sm">
						<div className="flex justify-between">
							...
							<span className={getGradeColor(getPerformanceGrade(metrics.fcp, 'fcp'))}>
								{metrics.fcp ? `${metrics.fcp}ms (${getPerformanceGrade(metrics.fcp, 'fcp')})` : 'Loading...'}
							</span>
						</div>
						<div className="flex justify-between">
							<span>LCP (最大内容绘制):</span>
							<span className={getGradeColor(getPerformanceGrade(metrics.lcp, 'lcp'))}>
								{metrics.lcp ? `${metrics.lcp}ms (${getPerformanceGrade(metrics.lcp, 'lcp')})` : 'Loading...'}
							</span>
						</div>
						<div className="flex justify-between">
							<span>FID (首次输入延迟):</span>
							<span className={getGradeColor(getPerformanceGrade(metrics.fid, 'fid'))}>
								{metrics.fid ? `${metrics.fid}ms (${getPerformanceGrade(metrics.fid, 'fid')})` : 'Loading...'}
							</span>
						</div>
						<div className="flex justify-between">
							<span>CLS (累积布局偏移):</span>
							<span className={getGradeColor(getPerformanceGrade(metrics.cls, 'cls'))}>
								{metrics.cls !== null ? `${metrics.cls} (${getPerformanceGrade(metrics.cls, 'cls')})` : 'Loading...'}
							</span>
						</div>
						<div className="flex justify-between">
							<span>TTFB (首字节时间):</span>
							<span className={getGradeColor(getPerformanceGrade(metrics.ttfb, 'ttfb'))}>
								{metrics.ttfb ? `${metrics.ttfb}ms (${getPerformanceGrade(metrics.ttfb, 'ttfb')})` : 'Loading...'}
							</span>
						</div>
					</div>
				</div>
				...
				<div className="mb-4">
					...
					<div className="space-y-2 text-sm">
						<div className="flex justify-between">
							...
							<span>{metrics.domContentLoaded ? `${metrics.domContentLoaded}ms` : 'Loading...'}</span>
						</div>
						<div className="flex justify-between">
							<span>页面完全加载:</span>
				...
				<div className="mt-4 border-t pt-4">
					...
					<div className="space-y-1 text-gray-600 text-xs">
						...
						{resources.some((r) => r.duration > 1000) && <div>• 存在加载时间超过1秒的资源，考虑优化或预加载</div>}
