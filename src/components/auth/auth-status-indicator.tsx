		...
		case 'avatar':
			return (
				<div className={`flex items-center gap-2 ${className}`}>
					...
					{showEmail && <span className="max-w-32 truncate text-gray-500 text-sm">{user.email}</span>}
				</div>
			);
...
export function SidebarAuthStatus({ className }: { className?: string }) {
	return (
		<AuthStatusIndicator variant="full" showEmail={true} showLogout={true} className={className} position="sidebar" />
	);
}
