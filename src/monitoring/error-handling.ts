...
import { AnalyticsError, ErrorCode, AnalyticsSystem, ErrorContext, RetryConfig, ApiResponse } from './types';
...
export class AnalyticsTrackingError extends AnalyticsError {
	...
	private getDefaultRetryConfig(): RetryConfig {
		return {
			...
			maxDelay: 10000,
			backoffFactor: 2,
			retryableErrors: ['NETWORK_ERROR', 'TIMEOUT', 'STORAGE_ERROR', 'QUOTA_EXCEEDED'],
		};
	}
}
...
export class AnalyticsErrorHandler {
	...
	public getErrorStatistics(): ErrorStatistics {
		...
		const recentErrors = this.errorBuffer.filter(
			(e) => Date.now() - e.context.timestamp.getTime() < 3600000, // Last hour
		);
	...
	private async performStorageCleanup(): Promise<void> {
		try {
			...
			const oldRetryItems = this.retryQueue.filter(
				(item) => Date.now() - item.nextRetryTime > 300000, // 5 minutes old
			);
			this.retryQueue = this.retryQueue.filter((item) => !oldRetryItems.some((old) => old.id === item.id));
		} catch (error) {
...
export class AnalyticsLogger {
	...
	public error(error: Error | AnalyticsError, data?: any): void {
		const message = error instanceof AnalyticsError ? `[${error.system}] ${error.message}` : error.message;
...
export class RetryMechanism {
	...
	public async executeWithRetry<T>(operation: () => Promise<T>, config: RetryConfig, context?: string): Promise<T> {
		let lastError: Error;
	...
	private isErrorRetryable(error: Error, retryableErrors: string[]): boolean {
		...
		return retryableErrors.some((code) => message.includes(code.toLowerCase().replace('_', ' ')));
	}
...
export function handleAsyncError<T>(promise: Promise<T>, errorHandler: (error: AnalyticsError) => void): Promise<T> {
	return promise.catch((error) => {
		const analyticsError =
			error instanceof AnalyticsError
				? error
				: createAnalyticsError('unknown' as AnalyticsSystem, 'OPERATION_FAILED', error.message, undefined, error);
