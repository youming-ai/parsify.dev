...
export class RealtimeAccessibilityScanner {
	...
	private async scanForViolations(scanContext: ScanContext): Promise<AccessibilityViolation[]> {
		...
		return violations.filter(
			(violation) => !this.configuration.ignorePatterns.some((pattern) => violation.element.matches(pattern)),
		);
	}
	...
	private scanInsufficientContrast(elements: Element[]): AccessibilityViolation[] {
		...
		const textElements = Array.from(elements).filter((el) =>
			['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'span', 'a', 'button', 'label', 'td', 'th'].includes(
	...
	private detectComponents(): Array<{
		name: string;
		type: ComponentAccessibilityResult['componentType'];
		elements: Element[];
	}> {
	...
	private getComponentName(element: Element): string {
		...
		while (parent && parent !== document.body) {
			if (parent.classList.length > 0) {
				const className = Array.from(parent.classList).find(
					(c) => c.includes('component') || c.includes('widget') || c.includes('control'),
				);
	...
	private getElementsToScan(): Element[] {
		...
		elements = elements.filter(
			(element) => !this.configuration.ignorePatterns.some((pattern) => element.matches(pattern)),
		);
	...
	private calculatePerformanceImpact(
		scanDuration: number,
		scanContext: ScanContext,
	): RealtimeScanResult['performanceImpact'] {
		return {
			scanOverhead: scanDuration,
	...
	private updateMetrics(result: RealtimeScanResult): void {
		...
		const totalScanTime =
			this.performanceMetrics.averageScanTime * (this.performanceMetrics.totalScans - 1) + result.scanDuration;
		...
		const totalImpact =
			this.performanceMetrics.performanceImpact * (this.performanceMetrics.totalScans - 1) +
			result.performanceImpact.scanOverhead;
		this.performanceMetrics.performanceImpact = totalImpact / this.performanceMetrics.totalScans;

		// Calculate scan efficiency
		this.performanceMetrics.scanEfficiency =
			this.performanceMetrics.violationsDetected / this.performanceMetrics.averageScanTime;
	}
	...
	private isAccessibilityRelevantElement(element: Element): boolean {
		...
		const isInteractiveElement = ['button', 'input', 'select', 'textarea', 'a', 'summary', 'details'].includes(tagName);
