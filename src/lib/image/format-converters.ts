...
import { useCanvasOperations, ImageDimensions, ProcessingOptions } from './canvas-operations';
...
export const useFormatConverter = () => {
   ...
   */
  const getOptimalQuality = useCallback(async (
...
export function getFormatInfo(format: string): {
  ...
  const formatInfo: Record<string, any> = {
    ...
    webp: {
      ...
      typicalUseCases: ['web images', 'photographs', 'images with transparency', 'responsive images']
    },
    bmp: {
      name: 'Bitmap',
      extensions: ['.bmp'],
