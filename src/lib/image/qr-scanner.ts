...
export const useQRScanner = (config: QRScannerConfig = {}) => {
  ...
  }, [checkCameraPermission, requestCameraPermission, initializeScanner, isScanning, scanTimeout]);
  ...
  }, [loadImage, enableBarcodeDetection, enablePreprocessing, maxScanAttempts]);

  /**
   * Scan with qr-scanner library
   */
  const scanWithQRScanner = useCallback(async (image: HTMLImageElement): Promise<QRScanResult[]> => {
      ...
      return [{
        ...
        metadata: {
          position: result.cornerPoints ? {
            ...
            width: Math.max(...result.cornerPoints.map((p: any) => p.x)) - Math.min(...result.cornerPoints.map((p: any) => p.x)),
            height: Math.max(...result.cornerPoints.map((p: any) => p.y)) - Math.min(...result.cornerPoints.map((p: any) => p.y))
          } : undefined
        }
      }];
    }
   ...
   */
  const scanWithBarcodeDetector = useCallback(async (image: HTMLImageElement): Promise<QRScanResult[]> => {
   ...
   */
  const scanWithPreprocessing = useCallback(async (
 ...
 */
export const validateQRCode = (data: string): {
  isValid: boolean;
  type?: string;
  errors?: string[];
} => {
