...
import { BatchConfig, BatchItem, type BatchProcessor, BatchStatistics } from './batch-processor';
...
export class BatchScheduler {
  ...
  public getStatistics(): {
    ...
    const completedBatches = batches.filter(
      (b) => b.status === 'completed' && b.startTime && b.endTime
    );
  ...
  private async processNextBatch(): Promise<void> {
    ...
    try {
      // Set up timeout
      const timeoutPromise = batch.maxDuration
  ...
  private garbageCollect(): void {
    ...
    if (completedBatches.length > this.config.maxCompletedBatches) {
      const toRemove = completedBatches.slice(
        0,
        completedBatches.length - this.config.maxCompletedBatches
      );
