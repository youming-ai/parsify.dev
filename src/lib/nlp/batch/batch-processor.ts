...
export class BatchProcessor {
   ...
   */
  public async process<T>(processor: (input: string, item: BatchItem) => Promise<T>): Promise<BatchItem<T>[]> {
      ...
      if (this.config.preserveOrder) {
        ...
        for (let i = 0; i < this.processingQueue.length; i += this.config.maxConcurrent) {
          ...
          results.push(...chunkResults.map((result, index) => {
            const item = this.items.get(chunk[index])!
            if (result.status === 'fulfilled') {
              return { ...item, ...result.value }
            } else {
              return { ...item, error: result.reason, status: 'failed' }
            }
          }))
        }
      } else {
        ...
        const processNext = (): void => {
          while (this.processingSet.size < this.config.maxConcurrent && this.processingQueue.length > 0) {
            const itemId = this.processingQueue.shift()!
            const promise = this.processItem(itemId, processor).then(() => {
   ...
   */
  private async processItem<T>(itemId: string, processor: (input: string, item: BatchItem) => Promise<T>): Promise<void> {
  ...
  public getStatistics(): BatchStatistics {
    ...
    const totalProcessingTime = this.startTime && this.endTime
      ? this.endTime.getTime() - this.startTime.getTime()
      : this.startTime
      ? Date.now() - this.startTime.getTime()
      : 0

    const itemsPerSecond = totalProcessingTime > 0
      ? (processedItems.length / totalProcessingTime) * 1000
      : 0
  ...
  public isComplete(): boolean {
    return this.items.size > 0 && this.items.values().every(item =>
      item.status === 'completed' || item.status === 'failed' || item.status === 'cancelled'
    )
  }
