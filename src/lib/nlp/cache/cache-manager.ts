...
export class CacheManager {
  ...
  public async warmup(
    ...
    for (const op of operations) {
      const success = await this.set(op.key, op.data, op.operation, {
        ...op.metadata,
        force: true,
      });
  ...
  public async import(data: any, operation?: string): Promise<void> {
    if (operation) {
      const cache = this.getCacheForOperation(operation);
      await cache.import(data as { entries: CacheEntry<any>[] });
    } else {
      for (const [name, cacheData] of Object.entries(data)) {
        const cache = this.caches.get(name);
        if (cache) {
          await cache.import(cacheData as { entries: CacheEntry<any>[] });
        }
      }
    }
  }
  ...
  private generateKey(input: any, operation: string): string {
    ...
    return strategy
      ? strategy.generateKey(input, operation)
      : `${operation}_${JSON.stringify(input)}`;
  }
  ...
  private recordMetrics(operation: string, type: string, latency: number, _success: boolean): void {
    ...
    this.metrics.operationsByType[operationKey] =
      (this.metrics.operationsByType[operationKey] || 0) + 1;
  }
  ...
  private getDefaultStrategies(): CacheStrategy[] {
    return [
      ...
      },
      {
        ...
        getTTL: (_result, _metadata) => 12 * 60 * 60 * 1000, // 12 hours
        getPriority: (entry) =>
          entry.metadata.accessCount / (Date.now() - entry.metadata.timestamp.getTime()),
      },
      {
        name: 'size-based',
        shouldCache: (_key, result, _metadata) => {
          const size = JSON.stringify(result).length;
          return size < 1024 * 1024; // 1MB limit
        },
        generateKey: (input, operation) => `${operation}_${this.simpleHash(JSON.stringify(input))}`,
        getTTL: (_result, _metadata) => 6 * 60 * 60 * 1000, // 6 hours
