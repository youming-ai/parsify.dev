...
export class ResultCache {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains('entries')) {
          ...
          entriesStore.createIndex('operation', 'metadata.operation', {
            unique: false,
          });
          entriesStore.createIndex('timestamp', 'metadata.timestamp', {
            unique: false,
          });
          entriesStore.createIndex('expiresAt', 'metadata.expiresAt', {
            unique: false,
          });
          entriesStore.createIndex('accessCount', 'metadata.accessCount', {
            unique: false,
          });
          entriesStore.createIndex('lastAccessed', 'metadata.lastAccessed', {
            unique: false,
          });
  ...
  public async getStatistics(): Promise<CacheStatistics> {
    ...
    const validEntries = entries.filter(
      (entry) => !entry.metadata.expiresAt || entry.metadata.expiresAt > now
    );
  ...
  public async getExpiredEntries(): Promise<CacheEntry[]> {
    ...
    const now = new Date();
    const entries = await this.getAllEntries();
    return entries.filter((entry) => entry.metadata.expiresAt && entry.metadata.expiresAt < now);
  }
  ...
  private async evictEntries(requiredSize = 0): Promise<number> {
    const entries = await this.getAllEntries();
    const now = new Date();
  ...
  private calculatePriority(entry: CacheEntry): number {
    ...
    const _timeSinceAccess = now.getTime() - entry.metadata.lastAccessed.getTime();
