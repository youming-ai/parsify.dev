...
export class ResultProcessor {
   ...
   */
  aggregateResults<T = any>(
    results: ProcessedResult<T>[],
    strategy: AggregationStrategy
  ): AggregatedResult[] {
    const aggregator = this.aggregators.get(strategy)
    if (!aggregator) {
      throw new Error(`Unsupported aggregation strategy: ${strategy}`)
    }
  ...
  private formatResult<T = any>(
    ...
    if (options.includeConfidence && processed.result.confidence !== undefined) {
      processed.result.confidence = Math.round(
        processed.result.confidence * Math.pow(10, options.precision || 3)
      ) / Math.pow(10, options.precision || 3)
    }
  ...
  private validateSentimentResult<T = any>(
    ...
    if (result.success && result.data) {
      ...
      if (data.score !== undefined && (typeof data.score !== 'number' || data.score < -1 || data.score > 1)) {
        errors.push({
  ...
  private validateSummarizationResult<T = any>(
    ...
    if (result.success && result.data) {
      ...
      if (data.compressionRatio !== undefined &&
          (typeof data.compressionRatio !== 'number' || data.compressionRatio < 0 || data.compressionRatio > 1)) {
        errors.push({
  ...
  private formatBatchResults<T = any>(
    results: ProcessedResult<T>[],
    format: OutputFormat
  ): ProcessedResult<T>[] {
    const formatter = this.formatters.get(format)
  ...
  private mergeIntersection<T = any>(resultSets: ProcessedBatchResult<T>[]): ProcessedBatchResult<T> {
    ...
    const commonResults = firstSet.results.filter(firstResult =>
      resultSets.every(rs =>
        rs.results.some(rsResult =>
          JSON.stringify(rsResult.result) === JSON.stringify(firstResult.result)
        )
      )
    )
    ...
    this.aggregators.set('average', {
      aggregate: (results) => {
        ...
        return Object.entries(groupedByType).map(([type, typeResults]) => ({
          type,
          confidence: typeResults.reduce((sum, r) => sum + (r.result.confidence || 0), 0) / typeResults.length,
  ...
  private groupByType<T = any>(results: ProcessedResult<T>[]): Record<string, ProcessedResult<T>[]> {
  ...
  private calculateStatistics<T = any>(results: ProcessedResult<T>[]): {
    if (results.length === 0) {
  ...
  private parseFromCSV(csvText: string): ProcessedResult<any>[] {
    ...
    return lines.slice(1).map(line => {
      ...
      return {
        operation: { type: values[0] },
        result: {
          success: values[1] === 'true',
          confidence: parseFloat(values[2]),
          processingTime: parseFloat(values[3]),
  ...
  private formatToXML<T = any>(batchResult: ProcessedBatchResult<T>): string {
    const xmlItems = batchResult.results.map(result =>
      this.formatSingleToXML(result)
    )
  ...
  private parseFromXML(xmlText: string): ProcessedResult<any>[] {
  ...
  private formatToText<T = any>(batchResult: ProcessedBatchResult<T>): string {
    return batchResult.results.map(result =>
      this.formatSingleToText(result)
    ).join('\n\n')
  }
...
`
  ...
  private parseFromText(textText: string): ProcessedResult<any>[] {
