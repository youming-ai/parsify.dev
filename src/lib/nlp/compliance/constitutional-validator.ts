...
export class ConstitutionalValidator {
  ...
  public async validateOperation(
    ...
    const performanceViolation = this.validatePerformance(
      operation.estimatedProcessingTime || 0
    )
    ...
    // Privacy validation
    const privacyViolation = this.validatePrivacy(operation.input, operation.sensitiveData || false)
  ...
  private validateResources(modelSize: number): ComplianceViolation | null {
    if (modelSize > this.constraints.resources.maxModelSize) {
      return {
        ...
        resolution: 'Use smaller model, implement model quantization, or use server-side processing',
      }
    }
