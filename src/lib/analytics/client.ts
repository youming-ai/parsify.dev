...
import { ANALYTICS_EVENTS, CLOUDFLARE_ANALYTICS_CONFIG, DEFAULT_ANALYTICS_CONFIG, RATE_LIMITS } from './config';
import type {
	APIUsageEvent,
	AnalyticsBatch,
	AnalyticsConfig,
...
export class CloudflareAnalyticsClient {
	...
	private hasDNT(): boolean {
		return (
			navigator.doNotTrack === '1' || (window as any).doNotTrack === '1' || (navigator as any).msDoNotTrack === '1'
		);
	}
...
export function createAnalyticsClient(config?: Partial<AnalyticsConfig>): CloudflareAnalyticsClient {
	if (!analyticsClient) {
