...
import { type MicrosoftClarityService, getMicrosoftClarityService } from './clarity';
import {
...
export function useAnalytics(config?: Partial<AnalyticsConfig>) {
	...
	return {
		...
		trackPerformance: useCallback(
			(params: Parameters<CloudflareAnalyticsClient['trackPerformance']>[0]) => client?.trackPerformance(params),
			[client],
		),
		trackInteraction: useCallback(
			(params: Parameters<CloudflareAnalyticsClient['trackInteraction']>[0]) => client?.trackInteraction(params),
			[client],
		),
		trackAPIUsage: useCallback(
			(params: Parameters<CloudflareAnalyticsClient['trackAPIUsage']>[0]) => client?.trackAPIUsage(params),
			[client],
		),
		trackCustomEvent: useCallback(
			(eventName: string, data: Record<string, any>, properties?: Record<string, string | number | boolean>) =>
				client?.trackCustomEvent(eventName, data, properties),
			[client],
		),
		updateConsent: useCallback((consent: Partial<AnalyticsConsent>) => client?.updateConsent(consent), [client]),
...
export function useDualAnalytics() {
	...
	const trackToolUsage = useCallback(
		(params: {
			...
			clarityAnalytics.trackToolUsage(params.toolId, params.toolName, params.action, {
