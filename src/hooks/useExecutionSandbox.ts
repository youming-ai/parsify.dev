...
interface SecurityViolation {
  type: 'forbidden_import' | 'malicious_pattern' | 'resource_limit' | 'timeout' | 'network_access' | 'file_access';
...
export const useExecutionSandbox = () => {
  ...
  const analyzeCode = useCallback((code: string, config: SecurityConfig): SecurityViolation[] => {
    ...
    while ((importMatch = importRegex.exec(code)) !== null) {
      ...
      if (config.blockedImports.some(blocked => moduleName.toLowerCase().includes(blocked.toLowerCase()))) {
        violations.push({
    ...
    if (code.includes('chr(') || code.includes('ord(') || code.includes('hex(') || code.includes('base64')) {
      violations.push({
        type: 'malicious_pattern',
        severity: 'medium',
  ...
  const createIsolatedEnvironment = useCallback((language: string, config: SecurityConfig) => {
  ...
  }, [validateAction, analyzeCode, startResourceMonitoring, getResourceUsage]);
  ...
  const abortAllExecutions = useCallback((): number => {
    let abortedCount = 0;
    for (const [id, controller] of abortControllers.current.entries()) {
      controller.abort();
      abortedCount++;
    }
