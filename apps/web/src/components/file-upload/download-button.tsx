		...
		<div className={cn('inline-block', className)}>
			<Button
				...
				className={cn(
					'relative',
					error &&
						'border-red-500 text-red-600 hover:border-red-600 hover:text-red-700'
				)}
			>
				{isDownloading ? (
					<>
			...
			{error && (
				<div className="mt-2 max-w-xs text-red-600 text-xs">{error}</div>
			)}
...
export const BatchDownloadButton: React.FC<BatchDownloadButtonProps> = ({
		...
		try {
			...
			for (let i = 0; i < validFiles.length; i++) {
				...
				if (file instanceof File) {
					...
					if (!response.ok) {
						throw new Error(
							`Download failed for ${file.name}: ${response.statusText}`
						);
					}
	...
	return (
		<div className={cn('inline-block', className)}>
			...
			>
				{isDownloading ? (
					<>
						...
						{showProgress
							? `${downloadProgress}%`
							: `Downloading ${validFiles.length} files...`}
					</>
				) : (
					<>
						<svg
			...
			{showProgress && isDownloading && (
				<div className="mt-2 max-w-xs text-gray-500 text-xs">
					Downloaded {Math.round((downloadProgress / 100) * validFiles.length)}{' '}
					of {validFiles.length} files
				</div>
			)}
		</div>
	);
