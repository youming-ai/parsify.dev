	...
	const handleFilesSelected = useCallback(
		async (selectedFiles: File[]) => {
			try {
				...
				const newUploadedFiles: UploadedFile[] = validation.validFiles.map(
					(file) => ({
				...
				const err =
					error instanceof Error ? error : new Error('Failed to process files');
				onError?.(err);
			}
		},
		[
		...
		<div className={cn('w-full space-y-6', layoutClasses[layout], className)}>
			...
			{!autoUpload && hasPendingFiles && (
				<div className="flex justify-center">
					<Button
						onClick={() =>
							uploadFiles(files.filter((f) => f.status === 'pending'))
						}
						disabled={isUploading}
						size="lg"
					>
