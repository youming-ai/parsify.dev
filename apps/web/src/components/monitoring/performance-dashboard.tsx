...
export const PerformanceDashboard: React.FC = () => {
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        <div className="h-8 w-8 animate-spin rounded-full border-gray-400 border-t-transparent border-b-2"></div>
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        <h1 className="mb-4 font-bold text-4xl text-gray-900 dark:text-white">性能监控仪表板</h1>
        <p className="text-gray-600 dark:text-gray-300">实时监控系统性能指标，及时发现和解决问题</p>
      </div>

      {/* 告警信息 */}
      {alerts.length > 0 && (
        <div className="mb-6">
          <h2 className="mb-4 font-semibold text-lg text-red-600">活跃告警 ({alerts.length})</h2>
                ...
                <CardHeader className="pb-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center space-x-2">
                      ...
                      {!alert.resolved && <Badge variant="destructive">未解决</Badge>}
                    </div>
                ...
                <CardContent>
                  ...
                  {alert.resolved && (
                    <div className="mt-2 text-green-600 text-sm">
                      已于 {new Date(alert.resolvedAt!).toLocaleString('zh-CN')} 解决
                    </div>
                  )}
                  {!alert.resolved && (
                    <button
      ...
      {/* 性能指标概览 */}
      <div className="mb-8">
        <h2 className="mb-4 font-semibold text-gray-900 text-lg dark:text-white">关键指标</h2>
            ...
            <Card key={index} className={getStatusColor(metric.status)}>
              ...
              <CardContent>
                <div className="flex items-baseline justify-between">
                  ...
                  <span className="text-gray-600 text-sm dark:text-gray-400">{metric.unit}</span>
                </div>
              </CardContent>
            </Card>
          ))}
