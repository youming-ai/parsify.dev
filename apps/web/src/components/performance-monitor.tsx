	...
	useEffect(() => {
		...
		const analyzeResources = () => {
			...
			const resourceTimings: ResourceTiming[] = resourceEntries
				.filter(
					(entry) => entry.name.includes('.') && !entry.name.includes('data:')
				)
				.map((entry) => ({
				...
				<div className="mb-4">
					...
					<div className="space-y-2 text-sm">
						<div className="flex justify-between">
							...
							<span
								className={getGradeColor(
									getPerformanceGrade(metrics.fcp, 'fcp')
								)}
							>
								{metrics.fcp
						...
						<div className="flex justify-between">
							...
							<span
								className={getGradeColor(
									getPerformanceGrade(metrics.ttfb, 'ttfb')
								)}
							>
								{metrics.ttfb
