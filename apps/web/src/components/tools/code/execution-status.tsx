								...
								<div className="flex items-center gap-3 rounded-lg bg-gray-50 p-3 dark:bg-gray-800">
									...
									<div>
										...
										<div className="text-gray-600 text-xs dark:text-gray-400">
											{formatTime(
												status === 'running' ? currentTime : executionTime
											)}
										</div>
									</div>
								</div>
						...
						<div className="rounded-lg border border-red-200 bg-red-50 p-3 dark:border-red-800 dark:bg-red-900/20">
							<div className="flex items-start gap-2">
								...
								<div className="flex-1">
									...
									<div className="mt-1 text-red-700 text-sm dark:text-red-300">
										{error}
									</div>
								</div>
							</div>
	...
	const getStatusIcon = () => {
		switch (status) {
			...
			case 'compiling':
				return (
					<Terminal
						className={cn(iconSize, getStatusColor(), 'animate-pulse')}
					/>
				);
			case 'running':
				return (
					<Loader2 className={cn(iconSize, getStatusColor(), 'animate-spin')} />
				);
			case 'completed':
