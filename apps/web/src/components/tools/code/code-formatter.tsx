...
export function CodeFormatter({
	...
	const [localOptions, setLocalOptions] =
		React.useState<CodeFormatOptions>(options);
				...
				return (
					<div className="space-y-2">
						...
						<Input
							...
							onChange={(e) =>
								updateOption(option.key, Number.parseInt(e.target.value, 10))
							}
							className="w-20"
						/>
					</div>
				...
				<CardHeader>
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-2">
							...
							>
								<Wand
									className={cn('mr-2 h-4 w-4', isFormatting && 'animate-spin')}
								/>
		...
		<div className={cn('flex flex-wrap gap-2', className)}>
			{Object.entries(FORMAT_PRESETS).map(([key, options]) => (
				<Button
					...
					size="sm"
					onClick={() =>
						onPresetChange(key as keyof typeof FORMAT_PRESETS, options)
					}
				>
