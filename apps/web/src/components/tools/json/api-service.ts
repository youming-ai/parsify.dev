...
export class JsonApiService {
  ...
  constructor(baseUrl: string = '/api/json') {
   ...
   */
  async validateFile(request: FileValidationRequest): Promise<FileValidationResponse> {
    try {
  ...
  private convertValidationResponse(response: any): JsonValidationResult {
    const errors: JsonValidationError[] = (response.errors || []).map((error: ValidationError) => ({
      line: error.line || 1,
