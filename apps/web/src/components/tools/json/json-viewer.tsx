...
function JsonTreeItem({
	...
	const [isExpanded, setIsExpanded] = React.useState(
		node.isExpanded ?? level < 3
	);

	const hasChildren = node.children && node.children.length > 0;
	const _isLastChild =
		node.key.endsWith(']') &&
		!Number.isNaN(Number.parseInt(node.key.split('[').pop() || '', 10));
	...
	const renderKey = () => {
		...
		return (
			<span
				className={cn(
					'cursor-pointer font-medium font-mono text-gray-700 hover:opacity-70'
				)}
				onClick={() => onCopy(node.path, node.value)}
