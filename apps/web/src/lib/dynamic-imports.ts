...
class DynamicImportManager {
  ...
  async import<T>(importFn: () => Promise<T>, config: DynamicImportConfig = {}): Promise<T> {
    ...
    // 开始加载
    const loadingPromise = this.loadModuleWithRetry<T>(
      importFn,
      retryCount,
      retryDelay,
      timeout,
    ...
    try {
      ...
      this.loadingPromises.delete(key)
      console.error('Dynamic import failed', error as Error, { moduleKey: key })
      throw error
    }
  }
   ...
   */
  async preloadBatch(
    imports: Array<{ importFn: () => Promise<any>; config?: DynamicImportConfig }>
  ): Promise<void> {
   ...
   * 清理缓存
   */
  clearCache(maxAge: number = 3600000): void {
    // 默认1小时
    const now = Date.now()
