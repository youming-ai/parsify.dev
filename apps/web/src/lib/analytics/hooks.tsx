...
export function useAnalytics(config?: Partial<AnalyticsConfig>) {
  ...
  return {
    ...
    trackToolUsage: useCallback((params: Parameters<CloudflareAnalyticsClient['trackToolUsage']>[0]) =>
      client?.trackToolUsage(params), [client]),
    trackPerformance: useCallback((params: Parameters<CloudflareAnalyticsClient['trackPerformance']>[0]) =>
      client?.trackPerformance(params), [client]),
    trackInteraction: useCallback((params: Parameters<CloudflareAnalyticsClient['trackInteraction']>[0]) =>
      client?.trackInteraction(params), [client]),
    trackAPIUsage: useCallback((params: Parameters<CloudflareAnalyticsClient['trackAPIUsage']>[0]) =>
      client?.trackAPIUsage(params), [client]),
    trackCustomEvent: useCallback((eventName: string, data: Record<string, any>, properties?: Record<string, string | number | boolean>) =>
      client?.trackCustomEvent(eventName, data, properties), [client]),
    updateConsent: useCallback((consent: Partial<AnalyticsConsent>) =>
      client?.updateConsent(consent), [client]),
...
export function useToolTracking(toolId: string, toolName: string) {
  ...
  const trackExecute = useCallback((processingTime?: number, inputSize?: number, outputSize?: number) => {
    trackToolUsage({
      toolId,
      toolName,
      action: 'execute',
  ...
  const trackValidate = useCallback((processingTime?: number, inputSize?: number) => {
    trackToolUsage({
      toolId,
      toolName,
      action: 'validate',
  ...
  const trackFormat = useCallback((processingTime?: number, inputSize?: number, outputSize?: number) => {
    trackToolUsage({
      toolId,
      toolName,
      action: 'format',
  ...
  const trackConvert = useCallback((processingTime?: number, inputSize?: number, outputSize?: number) => {
    trackToolUsage({
      toolId,
      toolName,
      action: 'convert',
  ...
  const trackError = useCallback((error: string, processingTime?: number) => {
    trackToolUsage({
      toolId,
      toolName,
      action: 'error',
...
export function usePerformanceTracking() {
  ...
  const trackCustomPerformance = useCallback((metrics: Parameters<CloudflareAnalyticsClient['trackPerformance']>[0]) => {
...
export function useInteractionTracking() {
  ...
  const trackClick = useCallback((elementId?: string, elementTag?: string, elementText?: string) => {
    trackInteraction({
  ...
  const trackFocus = useCallback((elementId?: string, elementTag?: string) => {
    trackInteraction({
  ...
  const trackNavigation = useCallback((targetUrl: string) => {
    trackInteraction({
...
export function useAPITracking() {
  ...
  const trackAPICall = useCallback((params: {
    endpoint: string
    method: string
    statusCode: number
    responseTime: number
...
export function useAnalyticsConsent() {
  ...
  const grantConsent = useCallback((level: 'minimal' | 'full' | 'custom' = 'full') => {
  ...
  const updateCustomConsent = useCallback((customConsent: Partial<AnalyticsConsent>) => {
    updateConsent({
      ...customConsent,
      timestamp: Date.now(),
    })
...
export function useEventTracking() {
  ...
  const createEventProps = useCallback((eventName: string) => {
    return {
