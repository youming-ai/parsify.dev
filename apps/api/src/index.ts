...
// Initialize database client and transaction services
app.use('*', async (c, next) => {
	try {
		// Import database classes
		const { DatabaseClient } = await import('./database/client');
		const { TransactionMonitor } = await import(
			'./database/transaction-monitoring'
		);
		const { TransactionManager } = await import('./database/transaction');

		// Initialize database client
		const dbClient = new DatabaseClient(c.env.DB, {
		...
		const transactionMonitor = new TransactionMonitor(dbClient);
		const transactionManager = new TransactionManager(dbClient);

		// Store in context
		c.set('db', c.env.DB);
		c.set('dbClient', dbClient);
		c.set('transactionMonitor', transactionMonitor);
		c.set('transactionManager', transactionManager);
		...
		return c.json(
			{
				...
				message: 'Unable to initialize database services',
				requestId: c.get('requestId'),
			},
			503
		);
