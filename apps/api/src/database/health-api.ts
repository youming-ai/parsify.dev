...
import { DatabaseHealthMonitor, createDatabaseHealthMonitor, HealthReport } from './health'
  ...
  private setupRoutes(): void {
        ...
        try {
          ...
          return c.json({
            ...
            queries: {
              ...
              successRate: metrics.queries.totalQueries > 0
                ? Math.round((metrics.queries.successfulQueries / metrics.queries.totalQueries) * 100)
                : 0,
    ...
    this.app.get('/health/ready', async (c) => {
      try {
        ...
        const isReady = healthResult.healthy &&
 ...
 */
export function addHealthRoutes(
  app: Hono,
