...
export interface SecurityViolation {
  type: 'cors' | 'csp' | 'missing_header' | 'invalid_header' | 'rate_limit' | 'custom'
  message: string
  path: string
  method: string
  origin?: string
...
export const securityMiddleware = (config: SecurityConfig = {}) => {
  return async (c: Context<{ Bindings: Env }>, next: Next) => {
    ...
    if (pathConfig.security?.customHeaders) {
      for (const [header, value] of Object.entries(pathConfig.security.customHeaders)) {
 ...
 */
function mergeSecurityConfig(config: SecurityConfig, environment: string): SecurityConfig {
 ...
 */
function getPathSpecificConfig(config: SecurityConfig, path: string): SecurityConfig {
 ...
 */
async function applySecurityHeaders(c: Context, config: SecurityConfig): Promise<void> {
  ...
  if (security.frameOptions) {
    if (security.frameOptions === 'ALLOW-FROM' && security.frameOptionsAllowFrom) {
      c.header('X-Frame-Options', `ALLOW-FROM ${security.frameOptionsAllowFrom}`)
    } else {
  ...
  // Permissions policy
  if (security.permissionsPolicy && Object.keys(security.permissionsPolicy).length > 0) {
 ...
 */
async function applyCSPHeaders(c: Context, config: SecurityConfig): Promise<void> {
 ...
 */
async function applyRateLimitHeaders(c: Context, config: SecurityConfig): Promise<void> {
  ...
  if (rateLimit) {
    ...
    if (rateLimit.retryAfter) {
      c.header('Retry-After', Math.ceil(rateLimit.retryAfter / 1000).toString())
    }
  }
}
...
export const createDevelopmentSecurity = (customConfig?: Partial<SecurityConfig>) => {
  return securityMiddleware(deepMerge(SecurityPresets.DEVELOPMENT, customConfig || {}))
}

export const createProductionSecurity = (customConfig?: Partial<SecurityConfig>) => {
  return securityMiddleware(deepMerge(SecurityPresets.PRODUCTION, customConfig || {}))
}

export const createPublicApiSecurity = (customConfig?: Partial<SecurityConfig>) => {
  return securityMiddleware(deepMerge(SecurityPresets.PUBLIC_API, customConfig || {}))
}

export const createAdminSecurity = (customConfig?: Partial<SecurityConfig>) => {
  return securityMiddleware(deepMerge(SecurityPresets.ADMIN, customConfig || {}))
}
...
export const createPermissionsPolicy = (policies: Record<string, string[]>): string => {
