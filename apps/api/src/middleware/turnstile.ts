...
export const turnstileMiddleware = (
	...
	return async (c: Context<{ Bindings: Env }>, next: Next) => {
		...
		const config: TurnstileConfig = {
			...
			...customConfig,
			siteKey:
				customConfig.siteKey ||
				c.env.TURNSTILE_SITE_KEY ||
				DEFAULT_CONFIG.siteKey,
			secretKey:
				customConfig.secretKey ||
		...
		try {
			...
			if (!token) {
				if (required) {
					return handleTurnstileError(
						c,
						TurnstileError.MISSING_TOKEN,
						requestId,
						onError
					);
				}
			...
			if (config.cachingEnabled) {
				const cachedResult = await getCachedValidationResult(
					cloudflare,
					token,
					config.cacheTTL
				);
			...
			if (!validationResult.success) {
				const error = mapTurnstileErrorCode(
					validationResult['error-codes']?.[0]
				);
			...
			if (config.ipBasedValidation || config.userAgentBasedValidation) {
				const botDetection = await performBotDetection(
					c,
					turnstileContext,
					config
				);
			...
			if (customValidation) {
				...
				if (!customValid) {
					if (required) {
						return handleTurnstileError(
							c,
							TurnstileError.VALIDATION_FAILED,
							requestId,
							onError
						);
					}
...
async function validateTurnstileToken(
	...
	while (attempt < maxAttempts) {
		try {
			...
			if (!response.ok) {
				throw new Error(
					`Turnstile validation failed: ${response.status} ${response.statusText}`
				);
			}
...
async function performBotDetection(
	...
	if (result.confidence >= threshold) {
		result.isBot = true;
		result.action =
			result.confidence >= threshold + 0.2 ? 'block' : 'challenge';
	}
...
function analyzeUserAgent(userAgent: string): {
	...
	const hasBrowserFeature = browserFeatures.some((feature) =>
		ua.includes(feature)
	);
...
async function getCachedValidationResult(
	...
	try {
		const cacheKey = `turnstile_validation:${token.substring(0, 20)}`;
		return await cloudflare.cacheGet<TurnstileValidationResponse>(
			'cache',
			cacheKey
		);
	} catch (error) {
...
async function logTurnstileEvent(
	...
	try {
		...
		await cloudflare.cacheSet(
			'analytics',
			`turnstile_event:${data.requestId}`,
			logData,
			{
				ttl: 86400, // 24 hours
			}
...
export const createLowProtection = (
	customConfig?: Partial<TurnstileMiddlewareOptions>
) => {
...
export const createMediumProtection = (
	customConfig?: Partial<TurnstileMiddlewareOptions>
) => {
	return turnstileMiddleware({
...
export const createHighProtection = (
	customConfig?: Partial<TurnstileMiddlewareOptions>
) => {
	return turnstileMiddleware({
...
export const createMaximumProtection = (
	customConfig?: Partial<TurnstileMiddlewareOptions>
) => {
	return turnstileMiddleware({
...
// Helper functions
export const getTurnstileContext = (
	c: Context
): TurnstileContext | undefined => {
