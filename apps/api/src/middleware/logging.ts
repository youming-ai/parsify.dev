...
export const loggingMiddleware = (options: LoggingMiddlewareOptions = {}) => {
  ...
  return async (c: Context<{ Bindings: Env }>, next: Next) => {
    ...
    const correlationId = enableCorrelationIds ? generateCorrelationId() : undefined
    ...
    c.text = (text: string, status?: number, headers?: Record<string, string>) => {
      responseStatus = status || 200
      responseData = text
    ...
    c.blob = (blob: Blob, status?: number, headers?: Record<string, string>) => {
      responseStatus = status || 200
...
async function updateRequestMetrics(
  ...
  try {
    ...
    // Hourly distribution
    dailyMetrics.requestsByHour[hour] = (dailyMetrics.requestsByHour[hour] || 0) + 1

    // Status code distribution
    const statusCode = logEntry.statusCode || 0
    dailyMetrics.requestsByStatus[statusCode] = (dailyMetrics.requestsByStatus[statusCode] || 0) + 1

    // Endpoint distribution
    const endpoint = logEntry.endpoint || 'unknown'
    dailyMetrics.requestsByEndpoint[endpoint] = (dailyMetrics.requestsByEndpoint[endpoint] || 0) + 1
 ...
 */
export const createDebugLogger = (options?: Partial<LoggingMiddlewareOptions>) =>
...
export const createErrorLogger = (options?: Partial<LoggingMiddlewareOptions>) =>
 ...
 */
export const createDevelopmentLogger = (options?: Partial<LoggingMiddlewareOptions>) =>
  loggingMiddleware({
    ...options,
    logLevel: 'debug',
    enableStructuredLogging: true,
 ...
 */
export const createProductionLogger = (options?: Partial<LoggingMiddlewareOptions>) =>
  loggingMiddleware({
    ...options,
    logLevel: 'info',
    enableStructuredLogging: true,
