		...
		try {
			...
			if (headers) {
				...
				if (check.retryAfter) {
					c.header(
						'Retry-After',
						Math.ceil(check.retryAfter / 1000).toString()
					);
				}
			}
...
async function checkSlidingWindowRateLimit(
	...
	try {
		...
		const currentUsage = state.requests.reduce(
...
export const createApiRateLimit = (
	customConfig?: Partial<RateLimitMiddlewareConfig>
) => {
...
export const createHeavyApiRateLimit = (
	customConfig?: Partial<RateLimitMiddlewareConfig>
) => {
	return rateLimitMiddleware({
...
export const createJobExecutionRateLimit = (
	customConfig?: Partial<RateLimitMiddlewareConfig>
) => {
	return rateLimitMiddleware({
