...
describe('Rate Limit Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    app.use('*', async (c, next) => {
      c.set('cloudflare', {
        cacheGet: (_namespace: string, key: string) => mockEnv.CACHE.get(key),
        cacheSet: (_namespace: string, key: string, value: any, options?: any) =>
          mockEnv.CACHE.put(key, JSON.stringify(value), options),
        cacheDelete: (_namespace: string, key: string) => mockEnv.CACHE.delete(key),
      })
  ...
  describe('Integration with Authentication', () => {
    it('should apply different rate limits for authenticated vs anonymous users', async () => {
      ...
      const anonymousRes = await app.request(
        '/api/test',
        {
          headers: {
  ...
  describe('Distributed Rate Limiting with KV', () => {
    it('should share rate limit state across multiple requests via KV', async () => {
      ...
      mockEnv.CACHE.put.mockImplementation(async (key: string, value: string, _options?: any) => {
      ...
      app.use(
        ...
        rateLimitMiddleware({
          ...
          window: 3600,
          distributed: true,
          keyGenerator: async _c => 'shared-key',
        })
      )
  ...
  describe('Route-specific Configurations', () => {
    it('should apply different limits and weights for different routes', async () => {
      ...
      expect(parseInt(lightRes.headers.get('X-Rate-Limit-Remaining') || '0', 10)).toBe(99)

      // Test heavy endpoint
      const heavyRes = await app.request('/api/heavy', {}, mockEnv)
      expect(heavyRes.status).toBe(200)
      expect(parseInt(heavyRes.headers.get('X-Rate-Limit-Remaining') || '0', 10)).toBe(90) // 100 - 10
    })
  })
