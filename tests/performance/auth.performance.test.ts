...
import { API_BASE_URL, AUTH_ENDPOINTS } from '../utils/endpoint-configs'
import {
...
describe('Auth API Performance Tests', () => {
  ...
  describe('GET /auth/validate - Session validation', () => {
    it('should handle unauthenticated session validation efficiently', async () => {
      ...
      console.log(
        `Auth validation performance: P95=${result.p95.toFixed(2)}ms, Success Rate=${((result.successfulRequests / result.totalRequests) * 100).toFixed(1)}%`
      )
    ...
    it('should maintain performance under high concurrency for session validation', async () => {
      ...
      Object.entries(concurrencyResults).forEach(([concurrency, result]) => {
        ...
        const maxAllowedTime = parseInt(concurrency, 10) > 50 ? 100 : endpoint.maxP95ResponseTime!
  ...
  describe('POST /auth/refresh - Token refresh', () => {
    it('should handle token refresh requests efficiently (should fail gracefully without auth)', async () => {
      ...
      console.log(
        `Auth refresh performance: P95=${result.p95.toFixed(2)}ms, Success Rate=${((result.successfulRequests / result.totalRequests) * 100).toFixed(1)}%`
      )
  ...
  describe('POST /auth/logout - Logout', () => {
    it('should handle logout requests efficiently (should fail gracefully without auth)', async () => {
      ...
      console.log(
        `Auth logout performance: P95=${result.p95.toFixed(2)}ms, Success Rate=${((result.successfulRequests / result.totalRequests) * 100).toFixed(1)}%`
      )
  ...
  describe('Auth API Rate Limiting', () => {
    it('should handle burst requests without performance degradation', async () => {
      ...
      console.log(
        `Auth burst test: P95=${burstResults.p95.toFixed(2)}ms, Success Rate=${((burstResults.successfulRequests / burstResults.totalRequests) * 100).toFixed(1)}%`
      )
      ...
      expect(burstResults.successfulRequests + burstResults.failedRequests).toBe(
        burstResults.totalRequests
      )
    })
    ...
    it('should maintain consistent response times under sustained load', async () => {
      ...
      batchResults.forEach((result, index) => {
        ...
        console.log(
          `Batch ${index + 1}: P95=${result.p95.toFixed(2)}ms, Success Rate=${(successRate * 100).toFixed(1)}%`
        )
      })
    })
  })
  ...
  describe('Auth Error Handling Performance', () => {
    it('should handle malformed requests efficiently', async () => {
      ...
      console.log(
        `Auth error handling: P95=${result.p95.toFixed(2)}ms, Success Rate=${((result.successfulRequests / result.totalRequests) * 100).toFixed(1)}%`
      )
    ...
    it('should handle requests with invalid headers efficiently', async () => {
      ...
      console.log(
        `Auth invalid headers: P95=${result.p95.toFixed(2)}ms, Success Rate=${((result.successfulRequests / result.totalRequests) * 100).toFixed(1)}%`
      )
