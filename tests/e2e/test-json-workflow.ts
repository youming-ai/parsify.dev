...
import { ToolWrapper } from '../../src/components/tools/tool-wrapper';
...
describe('Complete JSON Workflow E2E Tests', () => {
  ...
  describe('Complete User Journey', () => {
    it('should handle complete JSON workflow from input to export', async () => {
      ...
      await waitFor(() => {
        const treeView = screen.getByTestId('json-tree') ||
      ...
      await waitFor(() => {
        const codeOutput = screen.getByTestId('typescript-output') ||
  ...
  describe('Integration with Browser Features', () => {
    it('should support drag and drop file upload', async () => {
      ...
      const dropZone = screen.getByTestId('drop-zone') ||
                      screen.getByLabelText(/drop json file here/i);
