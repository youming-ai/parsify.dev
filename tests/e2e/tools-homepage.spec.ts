	...
	test.describe('Tool Cards and Interactions', () => {
		test('should display tool metadata correctly', async ({ page }) => {
			...
			const favoriteButton = firstCard.locator(
				'button[aria-label*="favorite"], button:has([data-testid="favorite-icon"])',
			);
		...
		test('should display tool features and tags', async ({ page }) => {
			...
			const featuresBadges = firstCard.locator(
				'.badge:has-text("Format"), .badge:has-text("Convert"), .badge:has-text("Validate")',
			);
	...
	test.describe('Advanced Filters', () => {
		test('should show advanced filters panel', async ({ page }) => {
			...
			if ((await advancedFiltersButton.count()) > 0) {
				...
				const filtersPanel = page
					.locator('.card')
					.filter({ has: page.locator('text=/Difficulty|Processing Type|Security Level/') });
