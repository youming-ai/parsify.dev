...
describe('Rate Limiting Integration Tests', () => {
  ...
  describe('Anonymous Rate Limiting', () => {
    it('should allow requests within anonymous rate limit', async () => {
      ...
      results.forEach((res, _index) => {
        ...
        expect(parseInt(res.headers.get('X-RateLimit-Remaining') || '0', 10)).toBeLessThan(20)
      })
    })
    ...
    it('should reject requests exceeding anonymous rate limit', async () => {
      ...
      const rejected = results.filter(r => r.status === 'fulfilled' && r.value.status === 429)
  ...
  describe('Authenticated Rate Limiting', () => {
    it('should provide higher rate limits for authenticated users', async () => {
      ...
      if (successful.length > 0 && successful[0].status === 'fulfilled') {
        ...
        expect(parseInt(res.headers.get('X-RateLimit-Limit') || '0', 10)).toBeGreaterThan(20)
      }
    })
  ...
  describe('Endpoint-Specific Rate Limiting', () => {
    it('should apply different limits for different endpoint types', async () => {
      ...
      const codeExecRequests = Array(8)
        .fill(null)
        .map((_, i) =>
          app.request(
      ...
      const jsonToolRequests = Array(8)
        .fill(null)
        .map((_, i) =>
          app.request(
            '/api/v1/tools/json/format',
    ...
    it('should apply stricter limits to resource-intensive operations', async () => {
      ...
      const successful = results.filter(r => r.status === 'fulfilled' && r.value.status === 200)
      const _rejected = results.filter(r => r.status === 'fulfilled' && r.value.status === 429)
  ...
  describe('Rate Limit Headers', () => {
    it('should include proper rate limit headers in responses', async () => {
      ...
      expect(parseInt(res.headers.get('X-RateLimit-Limit') || '0', 10)).toBeGreaterThan(0)
      expect(parseInt(res.headers.get('X-RateLimit-Remaining') || '0', 10)).toBeGreaterThanOrEqual(
        0
      )
      expect(parseInt(res.headers.get('X-RateLimit-Reset') || '0', 10)).toBeGreaterThan(0)
    })
