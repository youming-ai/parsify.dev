...
const MockJSONHeroViewer = ({ initialData = '', onPathClick, onSearch }: {
  initialData?: string;
  ...
  const generateTreeStructure = (data: any, path = '', level = 0) => {
    if (data === null || typeof data !== 'object') {
      return {
        ...
        level,
        hasChildren: false,
        isExpanded: expandedPaths.has(`${path}` || (level === 0 && expandedPaths.has('root'))
      };
    }
...
const TreeNode = ({ node, onPathClick, level }: {
  node: any;
  onPathClick: (path: string) => void;
  level: number;
}) => {
  ...
  return (
    <div style={{ marginLeft: indent }}>
      ...
      >
        {node.hasChildren ? (
          <span data-testid={`expand-icon-${node.path}`}>
            {node.isExpanded ? '▼' : '▶'}
          </span>
        ) : (
          <span>•</span>
        ...
        <span data-testid={`node-type-${node.path}`} style={{ marginLeft: 8, fontSize: '10px', color: '#999' }}>
          {node.type}
        </span>
      </div>
...
describe('JSON Hero Viewer Integration Tests', () => {
  ...
  describe('Tree Navigation and Expansion', () => {
    it('should expand and collapse tree nodes on click', async () => {
      ...
      render(<MockJSONHeroViewer initialData={JSON.stringify(nestedJson)} onPathClick={onPathClick} />);
  ...
  describe('Performance with Large JSON', () => {
    ...
    it('should maintain performance with frequent updates', async () => {
      let updateCount = 0;
      const onPathClick = () => { updateCount++; };

      const jsonWithArrays = {
        items: Array.from({ length: 100 }, (_, i) => ({ id: i, value: `Item ${i}` }))
      };

      render(<MockJSONViewer initialData={JSON.stringify(jsonWithArrays)} onPathClick={onPathClick} />);
