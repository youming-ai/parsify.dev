...
const THRESHOLDS = {
  ...
  cpuUsage: 0.80,            // 80%
  diskUsage: 0.90,           // 90%
  databaseConnections: 0.80, // 80% of max
...
function createPrometheusConfig() {
  ...
  fs.writeFileSync(configPath, `# Prometheus Configuration\n${JSON.stringify(config, null, 2).replace(/"/g, '')}`)
...
function createAlertRules() {
  ...
  const alertContent = `# Alert Rules for Parsify Platform\n${JSON.stringify(alerts, null, 2).replace(/"/g, '')}`
...
function createDockerCompose() {
  ...
  fs.writeFileSync(composePath, `# Docker Compose for Monitoring Stack\n${JSON.stringify(compose, null, 2).replace(/"/g, '')}`)
...
async function runHealthChecks(baseUrl) {
  ...
  if (mainHealth.success && mainHealth.data?.services) {
    for (const [service, status] of Object.entries(mainHealth.data.services)) {
  ...
  const monitor = async () => {
    try {
      ...
      if (healthResults.summary.failed > 0) {
        sendAlert(
          'health_check_failed',
          `${healthResults.summary.failed} health check(s) failed`,
          { url: baseUrl, results: healthResults }
        )
      }
